...
无符号整数类型（如 uint8、uint16、uint32、uint64 和 uint）在达到其类型能够表示的最大值时如果继续增加，就会从零开始重新计数，这种现象称为“回绕”（Wraparound）。
...
## 异常处理和日志记录
- 应合理使用 panic、recover、defer处理系统异常,避免出错信息输出到前端。
```go
defer func(){
	if r := recover(); r != nil {
...
# 5.加密解密
## 不得硬编码密码/密钥
在进行用户登录，加解密算法等操作时，不得在代码里硬编码密钥或密码，可通过变换算法或者配置等方式设置密码或者密钥。
```go
// bad
const (
	user = "dbuser"
